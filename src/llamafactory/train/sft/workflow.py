...
def run_sft(
    ...
    tokenizer_module = load_tokenizer(model_args) #分词器
    ...
    data_collator = SFTDataCollatorWith4DAttentionMask( #1.样本长度不同，padding 2.处理packing，对角attention mask和精度
        template=template,
        model=model if not training_args.predict_with_generate else None,
        pad_to_multiple_of=8 if training_args.do_train else None,  # for shift short attention，补到8的倍数
        label_pad_token_id=IGNORE_INDEX if data_args.ignore_pad_token_for_loss else tokenizer.pad_token_id,
        block_diag_attn=model_args.block_diag_attn,
        neat_packing=data_args.neat_packing, #确保不同样本间不能互相关注
    ...
    if training_args.predict_with_generate:#开放式的问答任务这两个指标都不够
        metric_module["compute_metrics"] = ComputeSimilarity(tokenizer=tokenizer)#翻译任务
    elif finetuning_args.compute_accuracy:
        metric_module["compute_metrics"] = ComputeAccuracy() #短答案
